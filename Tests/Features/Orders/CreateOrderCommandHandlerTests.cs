...
namespace Tests.Features.Orders
{
    public class CreateOrderCommandHandlerTests
    {
        ...
        public async Task Handle_WhenValidItems_ReturnsSuccess()
        {
            ...
            var item = new Item { Id = itemId, Name = "Test Item", ProductId = productId, Product = product };
            ...
            itemsRepo.Setup(
                x => x.UpdateItemStatus(It.IsAny<List<Guid>>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()))
                .Returns(Task.CompletedTask);
            ...
            itemsRepo.Verify(
                x => x.UpdateItemStatus(It.IsAny<List<Guid>>(), true, It.IsAny<CancellationToken>()),
                Times.Once);
        }
    }
}